...
class TestHashedPassword:
    ...
    def test_identity_equality(self):
        ...
        assert p == p, "Identity equality is broken"  # nosec

    def test_validation_eqality(self):
        """Verify validation check against cleartext."""
        p = HashedPassword("is_god")
        assert p == "is_god", "Password did not verify correctly"  # nosec
        assert p != "is_man", "Password did not verify correctly"  # nosec

    def test_creation_from_hash(self):
        """Test object creation from hash."""
        # hash is for 'is_god'
        p = HashedPassword(
            "$2b$12$j8hhjG.VS9wH3m5QM8BewOLy9gx7rfOhdONbA7kFVnpxvfFeImxGC", is_hash=True
        )  # noqa: E501
        assert p == "is_god", "Password did not verify correctly"  # nosec
        assert p != "is_man", "Password did not verify correctly"  # nosec
